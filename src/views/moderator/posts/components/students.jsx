...
import { db } from "/Users/omkardarekar/Documents/Be Project/credbud-web-frontend-final/src/firebase.js"; // Adjust the path based on your project setup
...
const handlePreview = (post) => {
  ...
  const userD = userData || { department: 'Computer' }; // fallback

useEffect(() => {
  const fetchPosts = async () => {
    try {
      const querySnapshot = await getDocs(
        collection(db, "Sinhgad/users/posts")
      );
      const fetchedPosts = [];
      querySnapshot.forEach((doc) => {
        fetchedPosts.push({ postId: doc.id, ...doc.data() });
      });
    ...
    <>
      ...
      >
        <VStack align="start" spacing={4}>
          ...
          {post.imageUrl && post.postType === 'PDF' && (
            <Box p={4} border="1px dashed" borderColor="gray.300" borderRadius="md" w="100%">
              <Text textAlign="center" fontSize="xl">📄 PDF Document</Text>
              <Text textAlign="center" fontSize="sm" color="gray.500">Click to view</Text>
            </Box>
          )}
          <Text fontWeight="semibold" fontSize="lg">{post.postText}</Text>
          <HStack spacing={3}>
            <Text fontSize="sm" color="gray.600">By: {post.ownerName}</Text>
            {post.isVerified && <Badge colorScheme="green">Verified</Badge>}
          </HStack>
          <HStack spacing={4}>
            <Badge colorScheme="purple">{post.postType}</Badge>
            <Badge colorScheme="blue">{post.department}</Badge>
          </HStack>
        </VStack>
      </MotionBox>

      <Modal isOpen={isOpen} onClose={onClose} size={post.postType === 'PDF' ? 'xl' : 'full'}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>{post.postText}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            {post.postType === 'Image' ? (
              <Image 
              ...
              <Box height="600px">
                {post.imageUrl ? (
                  <iframe 
                    src={`https://docs.google.com/viewer?url=${encodeURIComponent(post.imageUrl)}&embedded=true`}
                    width="100%" 
                    height="100%" 
                    style={{ border: 'none' }}
                    title={post.postText}
